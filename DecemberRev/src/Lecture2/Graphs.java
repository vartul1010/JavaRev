...
public class Graphs {
	...
	private class DjikstraPair implements Comparable<DjikstraPair> {
		...
		@Override
		public int compareTo(DjikstraPair djikstraPair) {
			return this.costSoFar - djikstraPair.costSoFar;
		}

		@Override
		public String toString() {
			return " [" + pathSoFar + " " + costSoFar + "]";
		}
	}

	public HashMap<String, DjikstraPair> djikstra(String srcvname) {

		HashMap<String, DjikstraPair> rv = new HashMap<>();
		PriorityQueue<DjikstraPair> heap = new PriorityQueue<>();
		...
		while (heap.size() > 0) {
			...
			for (String nbr : nbrs) {
				DjikstraPair np = rv.get(nbr);
				int oldCost = np.costSoFar;
				int newCost = rp.costSoFar + vces.get(np.name).nbrs.get(rp.name);
